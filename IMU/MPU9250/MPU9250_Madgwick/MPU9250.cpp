...
int MPU9250::calibrateGyro() {
  ...
  for (size_t i=0; i < _numSamples; i++) {
    readSensor();
    _gxbD += (getGyroX_rads() + _gxb)/((float)_numSamples);
    _gybD += (getGyroY_rads() + _gyb)/((float)_numSamples);
    _gzbD += (getGyroZ_rads() + _gzb)/((float)_numSamples);
    delay(20);
  }
  _gxb = (float)_gxbD;
  _gyb = (float)_gybD;
...
int MPU9250::calibrateAccel() {
  ...
  for (size_t i=0; i < _numSamples; i++) {
    readSensor();
    _axbD += (getAccelX_mss()/_axs + _axb)/((float)_numSamples);
    _aybD += (getAccelY_mss()/_ays + _ayb)/((float)_numSamples);
    _azbD += (getAccelZ_mss()/_azs + _azb)/((float)_numSamples);
    delay(20);
  }
  if (_axbD > 9.0f) {
    _axmax = (float)_axbD;
